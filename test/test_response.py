...
@pytest.mark.parametrize('r_model_trained, trained_responses', [({'idx': i}, {'idx': i}) for i in range(10)], indirect=True)
def test_response(r_model_trained, trained_responses, device):
    plot_dim = 0
    i, j = 0, 0
    ...
    assert np.allclose(E_x, trained_responses['E_x'], rtol=1.0e-3, atol=1.0e-4)
    assert np.allclose(E_y, trained_responses['E_y'], rtol=1.0e-3, atol=1.0e-4)
    assert np.allclose(I_x, trained_responses['I_x'], rtol=1.0e-3, atol=1.0e-4)
    assert np.allclose(I_y, trained_responses['I_y'], rtol=1.0e-3, atol=1.0e-4)
