...
def get_dataset():
    ...
    x_data, y_data_mean, y_data_sem, scale = load_exp_data(data_path, normalize=L/2)
...
def get_model(device='cpu'):
    ...
    # r_star
    _, y_data_base, _, _ = load_exp_data(f"{ENV['exp_path']}/baselines/base_by_dist.txt", normalize=L/2)
    ...
    ndim_s = 1
    Ls = [2*L]*ndim_s # we want our model to be twice the data length scale
